...
def plot(*ckpts: str,
    ...
    for label, marker in zip(['Ours (Distributed)', 'Ours (Separate)'] + baseline_labels, _MARKERS):
        if label == 'Ours (Distributed)':
            y = epoch_train_losses['Distributed'][1:max_epoch+1]
        elif label == 'Ours (Separate)':
            y = epoch_train_losses['Separate'][1:max_epoch+1]
        else:
    ...
    for label, marker in zip(['Ours (Distributed)', 'Ours (Separate)'] + baseline_labels, _MARKERS):
        if label == 'Ours (Distributed)':
            y = epoch_train_losses['Distributed'][offset:zoom_max_epoch+1]
        elif label == 'Ours (Separate)':
            y = epoch_train_losses['Separate'][offset:zoom_max_epoch+1]
        else:
            y = epoch_train_losses[label][offset:zoom_max_epoch+1]
        plot_with_ci(torch.arange(len(y))+offset, y, ax, label=label, smooth=False, marker=marker)
    ax.legend()
    ax.set_xticks(list(range(offset, zoom_max_epoch+1)))
    ax.set_xticklabels(list(range(offset, zoom_max_epoch+1)))
    ax.set_xlim(offset-0.5, zoom_max_epoch+0.5)
    ...
    for label, marker in zip(['Ours (Distributed)', 'Ours (Separate)'] + baseline_labels, _MARKERS):
        if label == 'Ours (Distributed)':
            ...
            y = test_accs['Separate'][1:max_epoch+1]
        else:
