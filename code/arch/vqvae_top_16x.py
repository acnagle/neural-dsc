...
class Encoder(nn.Module):
    def __init__(self, ch_in, ch_hidden, ch_residual, num_residual_layers, activation, cond_ch_in: int = None):
        ...
        self.res4 = vqvae.ResidualBlock(8 * ch_hidden, ch_residual, num_residual_layers, activation)
        ...
        self.conv4 = vqvae.Conv2d(8 * ch_hidden, 8 * ch_hidden, kernel_size=4, stride=2, padding=1)
...
class Decoder(nn.Module):
    def __init__(self, ch_in, ch_hidden, ch_out, ch_residual, num_residual_layers, activation, cond_ch_in: int = None):
        ...
        self.tconv1 = vqvae.ConvTranspose2d(8 * ch_hidden, 8 * ch_hidden, kernel_size=4, stride=2, padding=1)
        self.tconv2 = vqvae.ConvTranspose2d(8 * ch_hidden, 4 * ch_hidden, kernel_size=4, stride=2, padding=1)
        self.tconv3 = vqvae.ConvTranspose2d(4 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1)
        self.tconv4 = vqvae.ConvTranspose2d(2 * ch_hidden, 1 * ch_hidden, kernel_size=4, stride=2, padding=1)
        self.res1 = vqvae.ResidualBlock(8 * ch_hidden, ch_residual, num_residual_layers, activation)
        self.res2 = vqvae.ResidualBlock(4 * ch_hidden, ch_residual, num_residual_layers, activation)
        self.res3 = vqvae.ResidualBlock(2 *ch_hidden, ch_residual, num_residual_layers, activation)
        self.res4 = vqvae.ResidualBlock(1 *ch_hidden, ch_residual, num_residual_layers, activation)
...
class VqvaeTop16x(nn.Module):
    def __init__(self, *,
        ...
        """
            ...
            Latent    : (N, embedding_dim, ch_latent, H//32, W//16)
              -> Number of latents: ch_latent * (H//32) * (W//16)
        """
        super().__init__()
        ...
        self.hp.latent_shape = (ch_latent, image_shape[1]//32, image_shape[2]//16)
