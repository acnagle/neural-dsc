...
class AutoEncoder8x(nn.Module):
    def __init__(self, *,
        ...
        self.hp.latent_shape = (self.hp.embedding_dim * ch_latent, image_shape[1]//16, image_shape[2]//8)
    ...
    def forward(self, x, c=None):
        ...
        # Decoder
        x_rec = self.decoder(z_e, c=c)
        # x_rec = self.decoder(z_q, c=c)  # (N, C, H//2, W)
        # return x_rec, z_q, emb, z_e.view(*zs), idx
        # return x_rec, z_q, emb, z_e.view(*zs), idx
        return x_rec, z_e

    def decode_latent(self, z_e, c=None):
        if self.use_sideinfo:
            c = self.si_net(c)

        assert z_e.shape[1:] == tuple(self.hp.latent_shape)
        x = self.decoder(z_e, c=c)
        return x

    def get_autoencoder_param_count(self):
        return get_param_count(self)
