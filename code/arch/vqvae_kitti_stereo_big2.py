...
_HP_BASE = HParams(
    ...
    # Training
    batch_size          = 16,       # distributed across num_gpus
    learning_rate       = 5e-4,
...
class Encoder(nn.Module):
    def __init__(self, ch_in, ch_hidden, ch_residual, num_residual_layers, activation, cond_ch_in: int = None):
        ...
        self.conv3 = vqvae.Conv2d(4 * ch_hidden, 4 * ch_hidden, kernel_size=4, stride=2, padding=1)
...
class Decoder(nn.Module):
    def __init__(self, ch_in, ch_hidden, ch_out, ch_residual, num_residual_layers, activation, cond_ch_in: int = None):
        ...
        self.conv_in = vqvae.Conv2d(ch_in, 2 * ch_hidden, kernel_size=3, stride=1, padding=1)
        if cond_ch_in is None:
            self.has_sideinfo = False
            self.cond_conv_in = None
        else:
            ...
            self.cond_conv_in = nn.Sequential(
                vqvae.Conv2d(cond_ch_in, 2 * ch_hidden, kernel_size=3, stride=1, padding=1),
                vqvae.ResidualBlock(2 * ch_hidden, ch_residual, num_residual_layers, activation),
                vqvae.Conv2d(2 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1),
                vqvae.ResidualBlock(2 * ch_hidden, ch_residual, num_residual_layers, activation),
                vqvae.Conv2d(2 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1),
                vqvae.ResidualBlock(2 * ch_hidden, ch_residual, num_residual_layers, activation),
                vqvae.Conv2d(2 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1),
        ...
        self.tconv1 = vqvae.ConvTranspose2d(4 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1)
        self.tconv2 = vqvae.ConvTranspose2d(2 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1)
        self.tconv3 = vqvae.ConvTranspose2d(2 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1)
        self.res1 = vqvae.ResidualBlock(2 * ch_hidden, ch_residual, num_residual_layers, activation)
        self.res2 = vqvae.ResidualBlock(2 *ch_hidden, ch_residual, num_residual_layers, activation)
        self.res3 = vqvae.ResidualBlock(2 *ch_hidden, ch_residual, num_residual_layers, activation)
        self.cond_out = vqvae.Conv2d(2 * ch_hidden, ch_out, kernel_size=3, stride=1, padding=1)
...
class VqvaeKittiStereoBig2(nn.Module):
    def __init__(self, *,
                 image_shape,  # input image shape, e.g. (3, 128, 256)
                 codebook_bits: int,
                 ch_latent: int,
                 enc_si: bool,
                 dec_si: bool,
        ...
        """
            ...
            Latent    : (N, embedding_dim, ch_latent, H//8 W//8)
              -> Number of latents: ch_latent * (H//8) * (W//8)
        """
        assert image_shape[1] % 8 == image_shape[2] % 8 == 0
        ...
        self.hp.latent_shape = (ch_latent, image_shape[1]//8, image_shape[2]//8)
        ...
        self.z_proj = vqvae.Conv2d(4 * self.hp.ch_hidden, self.hp.embedding_dim * self.hp.ch_latent, kernel_size=1, stride=1, padding=0)
