...
class DecoderFC(nn.Module):
    ...
    def forward(self, x, c=None, c_orig=None):
        assert x.ndim == 2
        x = self.fc_in(x)
        if self.is_conditional:
            print(c.shape)
            print(self.fc_cond(c).shape)
            import sys; sys.exit()
            c = self.fc_cond(c)
        ...
        assert xc.shape == (len(x), self.d_hidden)
        xc = self.layers(xc)
        pred = torch.sigmoid(xc) - 0.5
        if self.is_conditional:
            return torch.clip((1. - self.alpha) * pred + self.alpha * c_orig, -0.5, 0.5)
        else:
            return pred
...
class SINet(nn.Module):
    def __init__(self, d_in, d_out, d_residual, num_residual_layers, activation):
        super().__init__()
        self.fc = nn.Linear(d_in, d_out)
        self.res = ResidualBlockFC(d_out, d_residual, 'gelu')

    def forward(self, x):
        x = self.res(self.fc(x))
        return x


class VqvaeDiscus(nn.Module):
    def __init__(self, *,
                 dim: int,
                 codebook_bits: int,
                 d_latent: int,
        ...
        self.decoder = DecoderFC(self.hp.embedding_dim * d_latent, self.hp.d_hidden, self.hp.d_residual, dim,
                                 d_cond=(self.hp.d_sideinfo if dec_si else None))
        self.quantizer = vqvae.VectorQuantizerEMA(2**codebook_bits, self.hp.embedding_dim)

        if self.use_sideinfo:
            self.si_net = SINet(dim, self.hp.d_sideinfo, self.hp.d_residual, self.hp.num_residual_layers, self.hp.activation)
        else:
            self.si_net = None
    ...
    def forward(self, x, c=None):
        ...
        # Decoder
        x_rec = self.decoder(z_q, c=c, c_orig=c_orig)
    ...
    def decode_indices(self, indices, c=None):
        ...
        x = self.decoder(z_q, c=c, c_orig=c_orig)
        return x
...
def test_stuff():
    ...
    model = VqvaeGaussian(dim=DIM, codebook_bits=4, d_latent=20, enc_si=False, dec_si=False)
