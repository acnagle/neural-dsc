    ...
    def __init__(self, ch_in, ch_hidden, ch_out, ch_residual, num_residual_layers, activation, cond_ch_in: int = None):
        ...
        if cond_ch_in is None:
            ...
            self.cond_conv_in = nn.Sequential(
                ...
                vqvae.Conv2d(1 * ch_hidden, 4 * ch_hidden, kernel_size=4, stride=2, padding=1),
            )

        self.tconv1 = vqvae.ConvTranspose2d(8 * ch_hidden, 2 * ch_hidden, kernel_size=4, stride=2, padding=1)
...
class VqvaeKittiStereo2x(nn.Module):
    def __init__(self, *,
        ...
        """
            ...
            Latent    : (N, embedding_dim, ch_latent, H//2 W//2)
              -> Number of latents: ch_latent * (H//2) * (W//2)
        """
        ...
        self.hp.latent_shape = (ch_latent, image_shape[1]//2, image_shape[2]//2)
        ...
        self.z_proj = vqvae.Conv2d(2 * self.hp.ch_hidden, self.hp.embedding_dim * self.hp.ch_latent, kernel_size=1, stride=1, padding=0)
